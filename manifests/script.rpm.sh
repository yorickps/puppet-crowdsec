...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='crowdsec_crowdsec'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/crowdsec_crowdsec.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='crowdsec_crowdsec'
  ...
  echo "Generating yum cache for crowdsec_crowdsec..."
  yum -q makecache -y --disablerepo='*' --enablerepo='crowdsec_crowdsec'

  echo "Generating yum cache for crowdsec_crowdsec-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='crowdsec_crowdsec-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh crowdsec_crowdsec
  zypper --gpg-auto-import-keys refresh crowdsec_crowdsec-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/crowdsec/crowdsec/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/crowdsec_crowdsec.repo
  else
    yum_repo_path=/etc/yum.repos.d/crowdsec_crowdsec.repo
  fi
